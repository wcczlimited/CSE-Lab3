                                ...
                                unsigned int xid_rep, char **b, int *sz)
{
    ...
    if(clt_xid_map.find(clt_nonce) == clt_xid_map.end())
    {
        clt_xid_map[clt_nonce] = xid_rep;
    }
    else
    {
        if(clt_xid_map[clt_nonce] < xid_rep)
            clt_xid_map[clt_nonce] = xid_rep;
    }
    //clean the window
    ...
    //determine the STSUS of the request
    ...
    if(xid <= clt_xid_map[clt_nonce])
        return FORGOTTEN;
    else
    {
        /*printf("xid > clt_xid_map[clt_nonce]\n");
        printf("clt_nonce %11d\n",clt_nonce);
        printf("xid %11d\n",xid);
        printf("%d > %d\n",xid,clt_xid_map[clt_nonce]);*/
        reply_t rep(xid);
